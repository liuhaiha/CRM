...
/**
 * ClassName:DefaultExcelStyleCreate
 */
public class DefaultExcelStyleCreate implements ExcelStyleCreate {
	 ...
	 * @throws Exception
	 */
	public void createRow(Object t, HSSFWorkbook workbook, HSSFSheet sheet, int rowIndex, String[] exportFields,
                          String dateFormat, Map<String, PropertyDescriptor> propertyMap, HSSFPatriarch patriarch) {
		...
		for (int i = 0; i < exportFields.length; i++) {
			...
			if (getMethod == null) {
				throw new RuntimeException("属性[" + pd.getName() + "]没有相对应的get方法");
			}
			...
			try {
				...
				throw new RuntimeException("执行属性方法[" + getMethod + "]出现错误");
			}
	...
	public void createCell(HSSFWorkbook workbook, HSSFSheet sheet, HSSFRow row, Object value, int rowIndex,
                           int cellIndex, String dateFormat, HSSFPatriarch patriarch) {
		...
		if (value == null) {
			...
			// 设置图片
			createPictureCellStyle(workbook, sheet, row, rowIndex, cellIndex, (byte[]) value, patriarch);
	...
	public void createPictureCellStyle(HSSFWorkbook workbook, HSSFSheet sheet, HSSFRow row, int rowIndex,
			int cellIndex, byte[] value, HSSFPatriarch patriarch) {
	...
	public void createDefaultCellStyle(HSSFCell cell, Object value) {
		...
		boolean matcher = textValue.matches(ExcelUtils.REGEX_NUMBER);
