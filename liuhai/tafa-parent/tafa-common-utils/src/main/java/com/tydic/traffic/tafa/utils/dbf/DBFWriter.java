...
public class DBFWriter extends DBFBase {
	...
	public DBFWriter(File dbfFile) throws DBFException {
		...
		try {
			...
			this.header.read(raf, this.characterSetName);
	...
	/**
	 * 
	 * Creates a new instance of DBFWriter.
	 *
	 * @param dbfFile
	 * @param characterSetName
	 * @throws DBFException
	 */
	public DBFWriter(File dbfFile, String characterSetName) throws DBFException {
		...
		try {
			...
			this.header = new DBFHeader();
			this.header.read(this.raf, characterSetName);
	...
	public void setFields(DBFField[] fields) throws DBFException {
		...
		try {
			...
			if (this.raf != null && this.raf.length() == 0) {
				 ...
				 * proceeding
				 */
				this.header.write(this.raf, this.characterSetName);
			}
		}
		catch (IOException e) {
	...
	/**
	 * 
	 * write:(Writes the set data to the OutputStream.). <br/>
	 * @param out
	 * @throws DBFException
	 * @since 1.0
	 */
	public void write(OutputStream out) throws DBFException {
		...
		try {
			...
			if (this.raf == null) {
				...
				this.header.write(outStream, this.characterSetName);
			...
			switch (this.header.fieldArray[j].getDataType()) {
				...
				case 'F':
					...
					if (objectArray[j] != null) {
						...
						dataOutput
								.write(Utils.doubleFormating((Double) objectArray[j], this.characterSetName,
										this.header.fieldArray[j].getFieldLength(),
