package com.tydic.traffic.tafa.utils.io;

import com.tydic.traffic.tafa.utils.file.FileUtils;
import com.tydic.traffic.tafa.utils.file.JarEntryInfo;
import com.tydic.traffic.tafa.utils.file.JarEntryResource;
import com.tydic.traffic.tafa.utils.file.NutResource;
...
public class StreamUtils {
	 ...
	 * @throws IOException
	 */
	public static Reader fileInr(File file) throws IOException {
	...
	/**
	 * 
	 * utf8r:(以UTF-8读取输入流). <br/>
	 * @param is
	 * @return
	 * @since 1.0
	 */
	public static Reader utf8r(InputStream is) {
		return new InputStreamReader(utf8filte(is), Charset.forName("UTF-8"));
	}
	...
	/**
	 * 
	 * _input:(获取File对象输入流,即使在Jar文件中一样工作良好). <br/>
	 * @param file
	 * @return
	 * @throws IOException
	 * @since 1.0
	...
	/**
	 * 
	 * makeJarNutResource:(这里用一句话描述这个方法的作用). <br/>
	 * @param file
	 * @return
	 * @since 1.0
	 */
	...
	public static NutResource makeJarNutResource(String filePath) {
		...
		try {
			...
			if (entry != null) {
				if (entry.getName().endsWith("/"))// 是文件夹
					return null;
