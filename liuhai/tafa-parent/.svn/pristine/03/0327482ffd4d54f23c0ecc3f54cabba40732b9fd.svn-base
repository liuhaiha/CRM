...
public class DBFReader extends DBFBase {
	...
	public DBFReader(InputStream in) throws DBFException {
		...
		try {
			...
			this.header.read(this.dataInputStream, this.characterSetName);
	...
	public DBFReader(InputStream in, String characterSetName) throws DBFException {
		this.isClosed = true;
		...
		try {
			...
			this.isClosed = false;
			this.header = new DBFHeader();
			this.header.read(this.dataInputStream, characterSetName);
			int t_dataStartIndex = this.header.headerLength - (32 + 32 * this.header.fieldArray.length) - 1;
			if (t_dataStartIndex > 0)
					...
					case 'D':
						...
						try {
							...
							GregorianCalendar calendar = new GregorianCalendar(
									...
									Integer.parseInt(new String(t_byte_month)) - 1, Integer.parseInt(new String(
											t_byte_day)));
