...
function(e) {
    ...
    i.prototype.tree = function(e, a) {
        ...
        function(a, n) {
            ...
            function() {
            	if(i.check){
            		var str='<input type="checkbox" name="checkTree" value="'+n.id+'" ';
            		if(n.flag){
            			str+="checked";
            		}
            		str+='/>';
            ...
            function() {
                return '<a href="' + (n.href || "javascript:;") + '" ' + (i.target && n.href ? 'target="' + i.target + '"': "") + ">" + ('<i class="layui-icon layui-tree-' + (l ? "branch": "leaf") + '">' + (l ? n.spread ? t.branch[1] : t.branch[0] : t.leaf) + "</i>") + ('<cite title="'+n.name+'">' + (n.name || "未命名") + "</cite></a>")
            } (), "</li>"].join(""));
    ...
    i.prototype.click = function(e, o) {
        ...
        function(e) {
            layui.jquery('.heightLight').removeClass();
            layui.jquery(this).addClass('heightLight');
            if(layui.jquery(this).siblings('input').attr('checked')){
            	layui.jquery(this).parent().find('input').each(function(){
            		layui.jquery(this).removeAttr('checked');
            	});
            }else{
            	layui.jquery(this).parents().each(function(){
            		if(layui.jquery(this).parents(e.elem)[0]!=undefined){
            			layui.jquery(this).siblings('input').attr('checked','checked');
            		}
            	});
            	layui.jquery(this).parent().find('input').each(function(){
            		layui.jquery(this).attr('checked','checked');
            	});
            }
            layui.stope(e),
            r.click(o)
