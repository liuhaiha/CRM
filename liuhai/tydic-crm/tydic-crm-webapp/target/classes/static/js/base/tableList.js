...
var totalPage = 0;
	var tableList = {
		...
		bindEvent : function() {
			$('#evaluateFile').on('click', this.evaluateFile)
						...
						success : function(json) {
							...
							if(data!=null){
								for (var i = 0; i < data.length; i++) {
									str += '<tr style="font-size:14px;height:35px;" name="tr">';
...
//			    		    $("tr[name='tr']").bind("click",function(){
//			    		    	var fid = $(this).children().first().children().first().val();
//			    		    	layer.open({
//			    		    		
//			    		    	});
//			    		    });
...
//							$('#totalPage').val(json.totalPage);
		...
		evaluateFile : function(){
			var checkArray = $('#model_list input:checkbox[name="fid"]:checked');
			if(checkArray.length==0){
				layer.msg('未选择文件');
				return;
			}else if(checkArray.length>1){
				layer.msg('不能同时对多个文件评分');
				return;
			}
			var fid = "";
			for (var i =0,len = checkArray.length; i < len; i++)
			{
				fid = $(checkArray[i]).val();
				
			}
			var star;
			$.ajax({
				url:'/crm/saleFile/getFileInfo',
				type:'post',
				data:{"fid":fid},
				dataType:'json',
				async:false,
				success:function(result){
					layer.msg("1");
					$("#docName").val(result.data.fname);
					$("#docStar").val(result.data.fstar);
					star = result.data.fstar;
				},
				error:function(){
					layer.msg("系统错误");
				}
			...
			layer.open({
				...
				yes:function(index,layero){
					var fstar = ((parseInt($("#docStar").val()) + parseInt(star)))/2;
					$.ajax({
						url:'/crm/saleFile/evaluate',
						type:'post',
						data:{"fid":fid,"fstar":fstar},
						dataType:'json',
						success:function(result){
							if(result.code == "1"){
								layer.closeAll();
								layer.msg('评分成功');
								tableList.queryFiles(pageNo);
							}
							else{
								layer.msg("评分失败");
							}
...
function topagepro(topge) {
	...
	if (topge == "tofirst") {
		...
		if($("#pageNumberPro").text() == $("#totalPagePro").text()){
			layer.msg("当前为最后一页",{offset:'40%'});
		}else{
			pageNumberPro = totalPage;
			$("#pageNumberPro").text(parseInt($("#totalPagePro").text()));
		}
	}
...
function changePagepro(obj) {
	...
	if (obj == "next") {
		if (pageNumberPro < totalPagePro) {
			...
			$("#pageNumber").text(pageNumberPro);
		} else if ($("#pageNumberPro").text() == $("#totalPagePro").text()) {
			layer.msg("当前为最后一页",{offset:'40%'});
		}
	} else if (obj == "last") {
