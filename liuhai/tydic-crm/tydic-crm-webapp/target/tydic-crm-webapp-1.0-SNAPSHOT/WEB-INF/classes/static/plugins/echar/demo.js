function initcalander(divid,dateList, currentMonth, currentDay) {
        ...
        var option = {
            ...
            tooltip:{
            	formatter:function(params){
            		if(params.data[2]==undefined || params.data[2]==null || params.data[2]=="")
            			return ;
            		return "当日事件：<br>"+params.data[2];
            	}
            },
            calendar: [{
                left: 'center',
                top: '50',
                cellSize: 'auto', // 设置日历格的大小，可支持设置不同高宽
            ...
            series: [{
                ...
                symbolSize: function(param, e,m){ // 核心2，把需要高亮的日期，宽度高度设置成和单元格宽度高度一样
                    if (e.value[0] == currentDay) {
                        return [$('#calander').width()/7-1,$('#calander').height()/5-9]
                    } else {
                        return 1
                    }
                },
                ...
                label: {
                    normal: {
                        ...
                        textStyle: {
                            fontSize: function(){
                            	return 14/$(window).width()*window.screen.availWidth
                            },
                            fontWeight: 600,
                            color: '#a00'
                        }
    ...
    calander.setOption(option);
        ...
        calander.setOption(option);
        window.onresize=calander.resize;
      //将有事件的方块变色，并且添加悬浮提示
        $.ajax({
    		async:true,
    		type:'GET',
    		url:'/crm/workDesk/getBacklogListInCurMonth',
    		success:function(json){
    			...
    			for(var i=0,len=data.length;i<len;i++){
    				visual.push([
    					echarts.format.formatTime('yyyy-MM-dd',data[i].ext_time.slice(0,10)),
    					'&nbsp;&nbsp;'+data[i].cname+'<br/>'
    				]);
    			}
    			var array=visual;
    	        if(array!=null){
    	        	for(var i=0,len=array.length;i<len;i++){
    	        		for(var j=0;j<heatmapData.length;j++){
    	        			if(heatmapData[j][0]==array[i][0]){
    	        				heatmapData[j][1]=1000;
    	        				if(heatmapData[j][2]==undefined){
    	        					heatmapData[j][2]='';
    	        				}
    	        				heatmapData[j][2]+=array[i][1];
    	        				break;
    	        			}
    	        		}
    	        	}
...
var Lunar = {
  ...
  lunarByBetween : function(year, between) {
    ...
    lunarArray.push(this.lunarYear(year),
            ...
            m,
            this.chineseNumber(e,m)); //天干地支年 生肖年 月份 日
  ...
  //中文日期
  chineseNumber : function(num,month) {
...
var currentMonth =year+"-"+Number(Number(month)+Number(1));

var currentDay=echarts.format.formatTime('yyyy-MM-dd',now.toLocaleDateString());

var lastDayInMonth=new Date(year,Number(Number(month)+Number(1)),0).getDate();
for(var i=1;i<=lastDayInMonth;i++){
	dateList.push([echarts.format.formatTime('yyyy-MM-dd',year+"/"+Number(Number(month)+Number(1))+"/"+i),Lunar.toLunar(year,Number(Number(month)+Number(1)),i)[6],""]);
}
