...
function searchBusName() {
	var busNameText = document.getElementById('pro_business_leader_name').value;
	if ($.trim(busNameText) == "") {
		$("#pro_bus").css("display", "none");
		return;
	}
	$.ajax({
		type     : "POST",
		url      : "../../base/getBusLeaderByName",
		dataType : "json",
		data     : {"ename": busNameText},
		success  : function(result) {
			if (result.length > 0) {
				$("#pro_bus").css("display", "");
				var dataArr = result;
				...
				var datahtml = "";
				for (var i = 0; i < result.length; i++) {
					datahtml += "<div onclick='busDivItemClick(this)' class='bg'>" + dataArr[i].ename + "</div>";
				}
				$("#pro_bus").html("<div style='height:60px;overflow:auto;border: 1px solid grey'>" + datahtml + "</div>");
			} else {
				$("#pro_bus").css("display", "none");
				//$("#ope_dec").css("display", "");
				//$("#ope_dec").html("<div style='height:60px;overflow:auto;border: 1px solid grey'><font color='red'>无搜索结果，请重试</font></div>");
...
function busDivItemClick(obj) {
	$("#pro_business_leader_name").val($(obj).text());
	$("#pro_bus").css("display", "none");
}
