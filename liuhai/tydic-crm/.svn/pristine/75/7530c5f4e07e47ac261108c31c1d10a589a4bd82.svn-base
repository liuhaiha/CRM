...
layui.define(['element', 'common'], function(exports) {
	...
	Tab.prototype.tabAdd = function(data) {
		...
		{
			// 移动端设置Div高度
			if (data.href.indexOf("base-customer.html") > -1)
			{
				$("#mainDiv").css("min-height","850px");
			}
			else if (data.href.indexOf("sale-new-meeting.html") > -1)
			{
				$("#mainDiv").css("min-height","1400px");
			}
			else if (data.href.indexOf("sale-tracking.html") > -1)
			{
				$("#mainDiv").css("min-height","1400px");
			}
			else if (data.href.indexOf("toEmpList") > -1)
			{
				$("#mainDiv").css("min-height","800px");
			}
			else
			{
		...
		if(tabIndex === -1) {
			if (globalTabIdIndex >= 1)
				{
				element.tabDelete(ELEM.tabFilter,0);
				globalTabIdIndex--;
				}
			globalTabIdIndex++;
